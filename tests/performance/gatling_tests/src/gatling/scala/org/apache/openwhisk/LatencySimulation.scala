...
class LatencySimulation extends Simulation {
  ...
  val actions: Seq[(String, String, String, String)] = Map(
    ...
    "python:default" -> (FileUtils
      .readFileToString(Resource.body("pythonAction.py").get.file, StandardCharsets.UTF_8), "woofie", ""),
      ...
      //     .create(code, "${action._1}", "${action._4}"))
        ...
        repeat(30) {
          ...
          // pause(pauseBetweenInvokes.milliseconds)
            exec(openWhisk("Warm ${action._1} invocation").authenticate(uuid, key).action("${action._3}").invoke("2.177"))
            // .exec { session => println(session); session }
        ...
        .repeat(30) {
          ...
          // invoker. If this is not finished before the next request arrives, a new cold-start has to be done.
          // pause(pauseBetweenInvokes.milliseconds)
            exec(openWhisk("med2 ${action._1} invocation").authenticate(uuid, key).action("${action._3}").invoke("2.302"))
            // .exec { session => println(session); session }
        ...
        .repeat(30) {
          ...
          // invoker. If this is not finished before the next request arrives, a new cold-start has to be done.
          // pause(pauseBetweenInvokes.milliseconds)
            exec(openWhisk("ligh1 ${action._1} invocation").authenticate(uuid, key).action("${action._3}").invoke("2.177"))
            // .exec { session => println(session); session }
