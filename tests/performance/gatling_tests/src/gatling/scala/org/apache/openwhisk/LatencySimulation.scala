      ...
      //     .create(code, "${action._1}", "${action._4}"))
        ...
        repeat(30) {
          ...
          // pause(pauseBetweenInvokes.milliseconds)
            exec(openWhisk("Warm ${action._1} invocation").authenticate(uuid, key).action("${action._3}").invoke("1.0"))
            // .exec { session => println(session); session }
        ...
        .repeat(30) {
          ...
          // invoker. If this is not finished before the next request arrives, a new cold-start has to be done.
          // pause(pauseBetweenInvokes.milliseconds)
            exec(openWhisk("med2 ${action._1} invocation").authenticate(uuid, key).action("${action._3}").invoke("5.0"))
            // .exec { session => println(session); session }
        ...
        .repeat(30) {
          ...
          // invoker. If this is not finished before the next request arrives, a new cold-start has to be done.
          // pause(pauseBetweenInvokes.milliseconds)
            exec(openWhisk("ligh1 ${action._1} invocation").authenticate(uuid, key).action("${action._3}").invoke("1.0"))
            // .exec { session => println(session); session }
