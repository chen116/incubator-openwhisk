...
//                              traceContext: Option[Map[String, String]] = None)
        ...
        val qtraceContext: Option[Map[String, String]] = if (meow_exectime.keySet.exists(_ == action.name.toString)) 
          {
            if ( meow_exectime.get(action.name.toString).get.last > 1000 && meow_quo.get(action.name.toString).get.last.toInt < 256)
            {
...
//meow_quo
  ...
  private def processResult(response: Either[ActivationId, WhiskActivation], tid: TransactionId): Unit = {
    ...
    logging.info(this, s"received result ack for '$aid'")(tid)
        ...
        meow_exectime.foreach({case (keyy, valuee) => logging.info(this,s"exectime meow $keyy $valuee")(tid)})   
