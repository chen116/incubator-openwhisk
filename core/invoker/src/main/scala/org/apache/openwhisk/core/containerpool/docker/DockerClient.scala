...
class DockerClient(dockerHost: Option[String] = None,
  ...
  def meow_DockerClient_update(id: ContainerId,quo:Int)(implicit transid: TransactionId): Future[Unit] =
    ...
    // runCmd(Seq("update", "--cpu-quota",quo.toString, "--cpu-period","10000",id.asString), config.timeouts.rm).map(_ => ())
    // runCmd(Seq("update", "--cpu-quota","10000", "--cpu-period","10000","--memory",quo.toString+"m",id.asString), config.timeouts.rm).map(_ => ())
    runCmd(Seq("update", "--cpu-quota",quo.toString, "--cpu-period","10000","--memory","200m",id.asString), config.timeouts.rm).map(_ => ())
